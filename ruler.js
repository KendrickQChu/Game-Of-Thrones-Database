module.exports = function(){
    ...
    function getRulers(res, mysql, context, complete){
        mysql.pool.query("SELECT Ruler_ID AS id, Major_Character.Name as Name, Succession_Type, Current_Ruler FROM `King/Queen_On_The_Iron_Throne` INNER JOIN Major_Character ON `King/Queen_On_The_Iron_Throne`.Character_ID = Major_Character.Character_ID", function(error, results, fields){
            if(error){
                res.write(JSON.stringify(error));
                res.end();
            }
            ...
            for(i = 0; i < results.length; ++i) {
                if(results[i].Succession_Type == 0) {
                    results[i].Succession_Type = "Inherited";
                }
                else if(results[i].Succession_Type == 1) {
                    results[i].Succession_Type = "Usurped";
                }
                if(results[i].Current_Ruler == 0) {
                    results[i].Current_Ruler = "False";
                }
                else if(results[i].Current_Ruler == 1) {
                    results[i].Current_Ruler = "True";
                }
            }
            context.ruler  = results;
            complete();
        });
    }
    ...
    function getCharacter(res, mysql, context, complete){
        ...
        mysql.pool.query("SELECT Major_Character.Character_ID AS id, Name FROM Major_Character", function(error, results, fields){
            //console.log(results);
            if(error){
                res.write(JSON.stringify(error));
                res.end();
    ...
    router.get('/', function(req, res){
        ...
        getRulers(res, mysql, context, complete);
        //getFamilies(res, mysql, context, complete);
        function complete(){
            callbackCount++;
            if(callbackCount >= 2){
                res.render('ruler', context);
            }
    ...
    router.post('/', function(req, res){
        ...
        var mysql = req.app.get('mysql');
        var sql = "INSERT INTO `King/Queen_On_The_Iron_Throne` (Character_ID, Succession_Type, Current_Ruler) VALUES (?,?,?)";
        var inserts = [req.body.Character_ID, req.body.Succession_Type, req.body.Current_Ruler];
