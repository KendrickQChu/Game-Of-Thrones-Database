module.exports = function(){
    ...
    function getRegion(res, mysql, context, complete){
        mysql.pool.query("SELECT Region_ID as id, Name FROM Region", function(error, results, fields){
            if(error){
    ...
    function getAlliance(res, mysql, context, complete){
        mysql.pool.query("SELECT Alliance_ID as id, Leader FROM Alliance", function(error, results, fields){
            if(error){
                res.write(JSON.stringify(error));
                res.end();
            }
    ...
    function getCharacters(res, mysql, context, complete){
        ...
        mysql.pool.query("SELECT Major_Character.Character_ID as id, Major_Character.Name as Name, Region.Name AS Home, Alliance.Leader AS Alliance, Major_Character.Seasons as Seasons, Major_Character.Trait as Trait FROM Major_Character LEFT JOIN Region ON Major_Character.Home = Region.Region_ID LEFT JOIN Alliance on Major_Character.Alliance = Alliance.Alliance_ID ORDER BY id", function(error, results, fields){
            //console.log(results);
            if(error){
                res.write(JSON.stringify(error));
                res.end();
    ...
    router.post('/', function(req, res){
        ...
        var mysql = req.app.get('mysql');
        var sql = "INSERT INTO Major_Character (Name, Home, Alliance, Seasons, Trait) VALUES (?,?,?,?,?)";
        var inserts = [req.body.Name, req.body.Home, req.body.Alliance, req.body.Seasons, req.body.Trait];
